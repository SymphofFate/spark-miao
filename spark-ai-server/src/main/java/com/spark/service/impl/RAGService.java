...
import com.spark.enums.SystemTypeEnum;
...
public class RAGService {
     ...
     * 基于知识库的智能问答
     */
    public Result askWithKnowledgeBase(ChatRequest request) {
        ChatMessage.ChatMessageBuilder builder = ChatMessage.builder();
        try {// 1. 从ES检索相关文档
            List<KnowledgeDocument> relevantDocs = elasticsearchService.semanticSearch(request.getQuestion(), 5);
            builder.requestCode(request.getCode())
            ...
            if (relevantDocs.isEmpty()) {
                ...
                systemPrompt = RAG_SYSTEM_PROMPT.replace("{context}", "")
                        .replace("{user}", SystemTypeEnum.getType(request.getCode()).getSystemPrompt());
            } else {
                // 2. 构建上下文
            ...
            // 4. 调用AI生成回答
            String answer = generateAnswerWithContext(systemPrompt, request.getQuestion(),request.getConversationId());
            ...
            ChatResponse response = ChatResponse.builder()
                    ...
                    .build();
            builder.answer("系统繁忙，请稍后重试")
                    .success(false)
                    .errorMessage(e.getMessage());
            return Result.success(response);
        }finally {
            messageDao.insert(builder.build());
        }
    }
    ...
    private String generateAnswerWithContext(String systemPrompt, String question, String conversationId) {
        ...
        try {
            ...
            dashScopeChatClient.prompt()
                    .system(systemPrompt)
                    .user(question)
                    .advisors(MessageChatMemoryAdvisor.builder(chatMemory).conversationId(conversationId).build())// 限制使用最近10条历史记录
                    .stream()
                    .content()
                    .toStream()
                    .forEach(content -> {
